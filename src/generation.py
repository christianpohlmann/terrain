...
class GenerationStrategy(object):
    ...
    def __get_neighbors(pt, shape):
        ...
        return [((a, b), wt) for ((a, b), wt) in neighbors if 0 <= a < w and b >= 0 and b < h]
...
class LinearFaultStrategy(GenerationStrategy):
    ...
    def __random_straight(self):
        ...
        if (x1 == x2):
            if x1 + LinearFaultStrategy._MODIFIER > self.width:
                x1 -= LinearFaultStrategy._MODIFIER
            else:
                x1 += LinearFaultStrategy._MODIFIER
        a = (y1 - y2) / (x1 - x2)
        b = y1 - a * x1
